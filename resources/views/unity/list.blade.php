...
<div class="content" style="padding-top: 0rem;">
                        ...
                        <td class="table-report__action w-56">
                            <div class="flex justify-center items-center">
                                <a href="javascript:;" data-tw-toggle="modal"
                                        data-tw-target="#overlapping-modal-preview_{{ $unity->id }}"
                                        class="flex items-center mr-3">
                                        <i data-lucide="check-square" class="w-4 h-4 mr-1"></i>Edit</a>

                                <a class="flex items-center text-danger delete-confirm" href="{{ route('unity.delete', $unity->id) }}"> <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i> Delete </a>
                            </div>
                        </td>
                    </tr>
 ...
 <!-- BEGIN: Modal Content -->
 <div id="overlapping-modal-preview_{{ $unity->id }}"  class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <!-- BEGIN: Modal Body -->
           <form method="POST" action="{{ route('unity.update' , $unity->id) }}"  >
                   @csrf
                       @method('PUT')
               <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                   <div class="col-span-12 sm:col-span-12"> <label for="modal-form-1" class="form-label">Nom du depot</label> </div>
                   <div class="col-span-12 sm:col-span-12">
                       <input id="name" name="name" type="text" value="{{ $unity->name }}" class="form-control" ></div>
                   @error('name')
